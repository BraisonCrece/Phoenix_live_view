defmodule InstaclownWeb.HomeLive do
  use InstaclownWeb, :live_view

  alias Instaclown.Posts
  alias Instaclown.Posts.Post

  def render(%{loading: true} = assigns) do
    ~H"""
    Instaclown is loading...
    """
  end
  ...
  def render(assigns) do
    ~H"""
    <h1 class="text-2xl text-center">🤡 InstaClown 🤡</h1>
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Instaclown.PubSub, "posts")
      form =
        %Post{}
        |> Post.changeset(%{})
        |> to_form(as: "post")
  ...
  def handle_event("save-post", %{"post" => post_params}, socket) do
    ...
    |> case do
      {:ok, post} ->
        ...
        Phoenix.PubSub.broadcast(Instaclown.PubSub, "posts", {:new, Map.put(post, :user, user)})
        {:noreply, socket}
  ...
  defp consume_files(socket) do
    consume_uploaded_entries(socket, :image, fn %{path: path}, _entry ->
      dest = Path.join([:code.priv_dir(:instaclown), "static", "uploads", Path.basename(path)])
      File.cp!(path, dest)
